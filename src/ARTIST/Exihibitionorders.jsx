...
export const Exihibitionorders = () => {
    ...
    useEffect(() => {
        let fetchData = async () => {
            let response = await axios.get(`https://artisan-market-backend.onrender.com/artist/exhviewproductorder/${id}`);
                            ...
                            {filteredData.map((item, index) => (
                                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-black" key={index}>
                                    ...
                                    <td className="flex pt-2 pb-2 gap-3">
                                        <Link to={`/artist/Assignexihibitionorders/${item.orders._id}`}>
