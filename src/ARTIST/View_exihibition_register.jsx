...
export const View_exihibition_register = () => {
    ...
    useEffect(() =>{
        let fetchData =async ()=>{
            let response =await axios.get('http://localhost:4000/artist/viewexihibitionregister')
    ...
    <div className='flex sm:gap-4 flex-wrap'>
        {data.map((item)=>(
                ...
                <div class=" max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                 ...
                 <div class="p-5">
                     <a href="#">
                         <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{item.exihibitionName} <br /> {item.name} <br /> {item.productName}</h5>
                     </a>
                     <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{item.exihibition.description}</p>
