                ...
                <div className='flex sm:gap-4 flex-wrap justify-evenly'>
                    {filterData().map((item) => (
                        <div key={item.exhibition?._id} className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <div className="p-5">
                                <a href="#">
                                    <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{item?.exhibition.exihibitionName} <br /><br />  {item?.registeration?.name} </h5>
                                    <span class="font-bold text-gray-700 dark:text-gray-300">START DATE: {new Date(item.exhibition.startdate).toLocaleDateString()}</span><br />
<span class="font-bold text-gray-700 dark:text-gray-300">END DATE: {new Date(item.exhibition.enddate).toLocaleDateString()}</span><br /><br />
                                </a>
