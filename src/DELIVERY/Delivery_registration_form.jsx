...
export const Delivery_registration_form = () => {
  ...
  let handleSubmit=async (event)=>{
    ...
    let response=await axios.post('http://localhost:4000/register',{...data,userType:'delivery'})
...
<div className='bg-sky-600 opacity-85 w-96 h-FIT'>
          <div><label className='p-10 font-bold text-[20px] justify-center flex gap-2'>
            ...
            <div className='p-3'>
              ...
              <input onChange={handleChange} name="id_proof" type="file" />
              </div>
              <div className='p-3'>
              ...
              <input type="text" name="Address" className='rounded' onChange={handleChange} placeholder='address' />
              </div>
              <div className='p-3'> 
              <label className=' text-gray-500 font-bold'>PASSWORD:</label><br />
              <input onChange={handleChange} name="password" className='rounded' type="text" placeholder='password' pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" />
              </div>
              <div className='p-3'>
              <label className=' text-gray-500 font-bold'>CONFIRM PASSWORD:</label><br />
              <input onChange={handleChange} name="confirm_password" className='rounded' type="text" placeholder='confirm password' pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" />
              </div>                            
          <div>
